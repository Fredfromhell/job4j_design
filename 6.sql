...
id serial primary key,
    name varchar(255),
    dap_id int references departments(id)
);
...
select * from departments d left join employees e on e.dep_id = d.id;
select d.id, d.name, e.id, e.name, e.dep_id from employees e right join departments d on d.id = e.dep_id;
...
select t1.name,t1.gender, t2.name, t2.gender, (t1.gender ||'+'|| t2.gender) as "m+w" from teens t1 cross join teens t2 where t1.gender != t2.gender;
